...
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Load overall scores',@action='START', @status='SUCCESS'
;with c as (
	select
		c.bvd_id
...
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Load overall scores',@action='END', @status='SUCCESS', @rowcount=@@ROWCOUNT

/* 2. Load the on-topic part */
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Load on-topic scores',@action='START', @status='SUCCESS'
;with c as (
	select
		c.bvd_id
	...
	from
		...
		investor_preference_family tf ON tf.inpadoc_family_id=cf.inpadoc_family_id
	where
...
FROM
	company b JOIN
	c ON c.bvd_id=b.bvd_id
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Load on-topic scores',@action='END', @status='SUCCESS', @rowcount=@@ROWCOUNT

/* 3. Load the frontier part */
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Load frontier scores',@action='START', @status='SUCCESS'
;with c as (
	select
		c.bvd_id
		, count(distinct tf.inpadoc_family_id) as on_topic_volume
	...
	from
		...
		[$(pwc_v2016a_005)].[aggregate].company_family cf ON cf.bvd_id=c.bvd_id LEFT OUTER JOIN
		[$(pw_v2016a_001)].patstat.family_metric fm ON fm.inpadoc_family_id=cf.inpadoc_family_id LEFT OUTER JOIN
		technology_family tf ON tf.inpadoc_family_id=cf.inpadoc_family_id
	where
		fm.priority_year >= (@py - 20)
		and
		fm.priority_year <= @py
