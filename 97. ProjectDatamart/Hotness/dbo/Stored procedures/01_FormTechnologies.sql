CREATE PROCEDURE [dbo].[01_FormTechnologies] (@tm_workspace_id int) AS
...
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Insert frontier technologies',@action='START', @status='SUCCESS'

truncate table technology;
INSERT INTO technology (label, topic_id, workspace_id,technology_group)
SELECT DISTINCT
	t.name
	, t.id as topic_id
	, t.workspace_id
...
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Insert frontier technologies',@action='END', @status='SUCCESS', @rowcount=@@ROWCOUNT

/* 2. Retrieve the inpadoc_family_ids the mention any of the words in the technologies */
EXEC [dbo].[log_addEntry] @component=@this_component, @message='Insert families belonging to frontier technologies',@action='START', @status='SUCCESS'

truncate table technology_family;
;with tr as (
	select
		t.technology_id
		, t.topic_id
	...
	where
		...
		and
		s.es_index='patstat'
) insert into technology_family (technology_id,inpadoc_family_id,number_of_citations,y3_citations,priority_year)
select distinct
	tr.technology_id
	, cast(e.[key] as int)
	, fm.family_citation
