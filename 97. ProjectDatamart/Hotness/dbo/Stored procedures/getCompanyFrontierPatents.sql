CREATE FUNCTION [dbo].[getCompanyFrontierPatents]
(
	@bvd_id varchar(25)
)
...
BEGIN
	...
	FROM
		...
		technology_family tf ON tf.inpadoc_family_id=cf.inpadoc_family_id JOIN
		technology t ON t.technology_id=tf.technology_id JOIN
		...
		(select distinct inpadoc_family_id from investor_preference_family) ipf ON ipf.inpadoc_family_id=cf.inpadoc_family_id
	where
		cf.bvd_id=@bvd_id
