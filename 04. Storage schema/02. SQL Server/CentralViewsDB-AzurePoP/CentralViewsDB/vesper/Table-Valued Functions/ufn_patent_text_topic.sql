CREATE FUNCTION [vesper].[ufn_patent_text_topic]
(
...
BEGIN
	...
	SET @es_task_id = (
			...
			WHERE
				...
				AND
				s.es_index='pw_v2016b_001.2' -- Arrange here that the used index is aligned with the rest of the data handler (i.e. that the inpadoc_family_id's correspond to the version of pw and pwc)
			ORDER BY
				r.[start] DESC
		)
...
/*	-- Alternative code to use the average score over the members in the patent family.
	...
	FROM
		...
		[$(pw_v2016b_001)].patstat.family_metric fm ON fm.inpadoc_family_id=cast([key] as int)
	where
		es_task_id=@es_task_id*/
	RETURN
END
