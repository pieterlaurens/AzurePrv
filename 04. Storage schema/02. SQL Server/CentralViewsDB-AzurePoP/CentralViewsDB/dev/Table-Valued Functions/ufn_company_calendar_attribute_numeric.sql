...
CREATE FUNCTION dev.[ufn_company_calendar_attribute_numeric]
(
...
BEGIN
	...
	BEGIN
		...
		from
			[$(scd_v2017_004)].dbo.orbis_financials f JOIN
			dev.calendar c ON c.period_year=f.y
		where
	...
	BEGIN
		...
		FROM
			[$(scd_v2017_004)].dbo.orbis_basic f CROSS JOIN
			(SELECT TOP 10 * FROM dev.calendar WHERE period_type=@period_type ORDER BY period_date DESC) c
	END
