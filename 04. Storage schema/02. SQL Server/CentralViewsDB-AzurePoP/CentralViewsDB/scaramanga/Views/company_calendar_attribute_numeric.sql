...
CREATE VIEW scaramanga.[company_calendar_attribute_numeric]
	AS
	SELECT
		c.id as calendar_id
		, c.period_type
	...
	FROM
		...
		scaramanga.calendar c ON c.bloomberg_period_id=epm.period_id
	UNION
	select
		p.calendar_id
		, p.period_type
	...
	from
		...
		from
			...
			scaramanga.calendar c ON c.period_year=p.[value]
