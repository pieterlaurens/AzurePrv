CREATE VIEW [boris].[company_aggregate_attribute] AS
SELECT
...
FROM
	...
	[$(scd_v2016_008)].dbo.financials_lay f ON ob.bvd_id=f.bvd_id LEFT OUTER JOIN
	(select
		acquirer_bvd_id as bvd_id
		, sum(do.[deal_value]) as total_acquisition_value
		, count(distinct da.deal_id) as number_of_acquisitions
	from
		[$(sdd_v2016_005)].dbo.deal_acquirer da JOIN
		[$(sdd_v2016_005)].dbo.deal_overview do ON do.deal_id=da.deal_id
	WHERE
