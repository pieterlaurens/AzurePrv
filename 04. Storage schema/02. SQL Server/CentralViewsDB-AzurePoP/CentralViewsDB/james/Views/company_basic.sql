...
CREATE VIEW [james].[company_basic] AS
SELECT
	c.ticker as bvd_id
	, isnull(company_name,c.ticker) as company_name
	, postal_code
	, city
...
FROM
	(select * from [$(idr_linktables_v2017_002)].dbo.linktable_ticker_to_cik where cik is not null) c LEFT OUTER JOIN
	[$(idr_linktables_v2017_002)].dbo.linktable_ticker_to_bvd lb ON lb.ticker=c.ticker LEFT OUTER JOIN
	[$(scd_v2017_002)].dbo.orbis_basic ob ON lb.bvd_id=ob.bvd_id LEFT OUTER JOIN
	[$(pwc_v2016b_002)].[aggregate].company_ip_metric cim ON cim.bvd_id=lb.bvd_id LEFT OUTER JOIN
	[$(scd_v2017_002)].dbo.financials_lay layf ON layf.bvd_id=lb.bvd_id LEFT OUTER JOIN
	[$(scd_v2017_002)].dbo.orbis_stock os ON os.bvd_id=lb.bvd_id