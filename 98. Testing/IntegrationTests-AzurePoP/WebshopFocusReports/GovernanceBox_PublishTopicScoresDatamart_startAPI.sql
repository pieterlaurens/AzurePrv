...
DECLARE @CalculateTrendFromYear int = 2012
		...
		, @TruncateReferenceTablesBeforeLoad bit = 1
...
DECLARE @SOLUTION nvarchar(255) = 'GovernanceBox'
		...
		, @API nvarchar(512) = 'PublishTopicScoresDatamart'
...
BEGIN
	SELECT @TopicDbServer = 'NLAMS10859', @TopicDbCatalog = 'pw_v2016b_001'
END
ELSE IF @Application NOT IN ('Risk', 'Strategy', 'BusinessProcess', 'Deal', 'Technology')
BEGIN
	SET @logmessage = CONVERT(nvarchar, GETDATE(), 121) + ' No valid application specified; application should be in {Risk, Strategy, BusinessProcess}'
	RAISERROR (@logmessage, 0, 1) WITH NOWAIT
...
SET @json_config = REPLACE(@json_config, '$(TruncateBeforeLoad)',  CASE WHEN @TruncateReferenceTablesBeforeLoad = 1 THEN 'true' ELSE 'false' END)
